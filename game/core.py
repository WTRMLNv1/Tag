...
def run_game(screen, selected_map):
    ...
    # Create players
    blue = Player(blue_img, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2, player_settings())  
    red = Player(red_img, SCREEN_WIDTH // 4, SCREEN_HEIGHT // 4, player_settings())    
    players = [blue, red]
    ...
    while running:
        ...
        # Handle tag logic
        if pygame.sprite.collide_rect(blue, red) and (now - last_tag_time > COOLDOWN_TIME): 
    ...
    screen.blit(text, (SCREEN_WIDTH // 2 + 170, SCREEN_HEIGHT // 2 + 70)) 
    pygame.display.flip()
...
def load_map(selected_map, images_path):
    if selected_map == 1:
        ...
        platforms = [
            ...
            Platform(300, 550, 200, 20, str(images_path / "200.png")),
            Platform(600, 550, 150, 20, str(images_path / "150.png"))
        ]
    else:
...
def get_remaining_time(start_time):
    ...
    return max(0, TIMER_DURATION // 1000 - elapsed) 
...
def player_settings():
    return {
        ...
        "gravity": GRAVITY, 
        "boost_amount": SPEED_BOOST_AMOUNT, 
        "boost_duration": SPEED_BOOST_DURATION, 
        "max_boosts": MAX_SPEED_BOOSTS 
    }
